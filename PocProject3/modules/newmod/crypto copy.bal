...
function decodePrivateKey2() returns crypto:PrivateKey|error {
...
function decodePublicKey2() returns crypto:PublicKey|error {
...
function encrypt2() returns error? {
...
public function newCrypto2() returns error? {
    check hash();
    check hmac();
    check sign();
    check encrypt();
