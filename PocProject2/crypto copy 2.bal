...
function decodePrivateKey3() returns crypto:PrivateKey|error {
...
function decodePublicKey3() returns crypto:PublicKey|error {
...
function encrypt3() returns error? {
...
public function newCrypto3() returns error? {
    check hash();
    check hmac();
    check sign();
    check encrypt();
